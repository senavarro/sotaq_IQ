...
const getPhonemeTip = (phoneme) => {
  const tips = {
    ...
    'ð': "Língua entre os dentes vibrando (não é som de 'd' nem 'z').", // th in this
    'ɹ': "Enrole a língua para trás, tipo sotaque caipira (não raspe a garganta).", // r in red
    'ɪ': "Som de 'i' bem curto e relaxado, quase puxando pro 'ê'.", // i in sit
    'i:': "Som de 'i' bem longo e esticado, como um sorriso.", // ee in see
    'æ': "Abra bem a boca, é um som entre o 'a' e o 'e'.", // a in cat
    'h': "Solte o ar como um suspiro cansado (o 'h' não é mudo!).", // h in hot
...
export default function SotaQApp() {
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try {
        await navigator.share({ title: 'SotaQ AI', text: shareText, url: appUrl });
      } catch (err) {
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        if (wordData.length === 0 || heardStr.trim() === "") {
            setFeedback({ score: 0, stars: 0, fluency: 0, prosody: 0, heard: "Nenhuma voz detectada.", errors: [], worstWord: null, msg: "Não ouvimos você! 🛑" });
        ...
        wordData.forEach(w => {
            if (w.phonemes) {
                w.phonemes.forEach(p => allPhonemes.push({ word: w.word, sound: p.sound, score: Number(p.score) || 100 }));
            }
        });

        let lowestPhonemeScore = rawAverage;
        if (allPhonemes.length > 0) {
            const worstPhonemeObj = allPhonemes.reduce((prev, curr) => prev.score < curr.score ? prev : curr);
        ...
        const strictErrors = wordData.filter(w => (Number(w.accuracy) || 100) < 90).map(w => {
            ...
            if (w.phonemes && w.phonemes.length > 0) {
                worstPhoneme = w.phonemes.reduce((prev, curr) => (Number(prev.score) < Number(curr.score)) ? prev : curr);
            }
            return { ...w, worstPhoneme };
        });
        
        if (strictErrors.length > 0) {
            strictScore -= (strictErrors.length * 6); 
        }
      ...
      <div style={{ maxWidth: '450px', margin: '0 auto', padding: '0 20px', paddingBottom: '40px' }}>
        ...
        <div style={{ background: 'white', borderRadius: '30px', padding: '30px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.05)', textAlign: 'center', marginBottom: '20px' }}>
           ...
           {isCustomMode ? (
             <textarea 
               ...
               placeholder="Digite uma frase (curta)..." maxLength={100}
        ...
        {feedback && !isProcessing && (
          <div style={{ background: 'white', padding: '20px', borderRadius: '24px', textAlign: 'center', marginBottom: '20px', border: '2px solid #e2e8f0', animation: 'fadeIn 0.5s' }}>
            ...
            <p style={{ fontWeight: '900', margin: '0 0 5px 0', color: '#1a2a6c', fontSize: '1.4rem' }}>{feedback.score}% Precisão</p>
            <p style={{ color: '#64748b', fontSize: '0.9rem', fontStyle: 'italic', margin: '0 0 15px 0' }}>🗣️ A IA ouviu: "{feedback.heard}"</p>
            ...
            {feedback.errors && feedback.errors.length > 0 ? (
              <div style={{ marginTop: '15px', padding: '10px', background: '#fef2f2', borderRadius: '12px', border: '1px solid #fca5a5' }}>
                <p style={{ fontSize: '0.8rem', fontWeight: 'bold', color: '#ef4444', margin: '0 0 10px 0' }}>⚠️ Raio-X da Pronúncia:</p>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {feedback.errors.map((err, idx) => (
                    <div key={idx} style={{ background: '#ef4444', color: 'white', padding: '8px 12px', borderRadius: '8px', textAlign: 'left' }}>
                      <span style={{ display: 'block', fontSize: '1rem', fontWeight: 'bold' }}>{err.word} ({Math.round(err.accuracy)}%)</span>
                      {err.worstPhoneme && (
                        <div style={{ marginTop: '5px', background: 'rgba(0,0,0,0.15)', padding: '6px', borderRadius: '6px' }}>
                          <span style={{ display: 'block', fontSize: '0.75rem', fontWeight: 'bold', color: '#fca5a5' }}>
                            Som que te derrubou: /{err.worstPhoneme.sound}/ ({Math.round(err.worstPhoneme.score)}%)
                          </span>
                          <span style={{ display: 'block', fontSize: '0.75rem', marginTop: '3px' }}>
                            💡 Dica: {getPhonemeTip(err.worstPhoneme.sound)}
                          </span>
                        </div>
                      )}
                    </div>
